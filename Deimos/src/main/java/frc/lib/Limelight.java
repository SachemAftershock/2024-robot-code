...
/**
	...
	public FluidicalPoseInfo getBotPoseViaNetworkTables() {
		...
		try {
			Optional<Alliance> ally = DriverStation.getAlliance();
			if ((ally.get() != Alliance.Red) && (ally.get() != Alliance.Blue)) throw new Exception("Alliance not Found!");

			String allianceSidePose = ally.get() == Alliance.Blue ? "botpose_wpiblue" : "botpose_wpired";
