...
public class AftershockChoreo {
  private static final Gson gson = new Gson();

  /** Default constructor. */
  public AftershockChoreo() {}
   ...
   */
  public static AftershockChoreoTrajectory getTrajectory(String trajName) {
   ...
   */
  public static ArrayList<AftershockChoreoTrajectory> getTrajectoryGroup(String trajName) {
    ...
    var trajs = new ArrayList<AftershockChoreoTrajectory>();
    for (int i = 1; i <= segmentCount; ++i) {
      File traj = new File(traj_dir, String.format("%s.%d.traj", trajName, i));
      AftershockChoreoTrajectory trajectory = loadFile(traj);
      if (trajectory == null) {
  ...
  private static AftershockChoreoTrajectory loadFile(File path) {
    try {
      var reader = new BufferedReader(new FileReader(path));
      AftershockChoreoTrajectory traj = gson.fromJson(reader, AftershockChoreoTrajectory.class);
  ...
  public static Command choreoSwerveCommand(
      AftershockChoreoTrajectory trajectory,
  ...
  public static Command choreoSwerveCommand(
      AftershockChoreoTrajectory trajectory,
      Supplier<Pose2d> poseSupplier,
      AftershockChoreoControlFunction controller,
      Consumer<ChassisSpeeds> outputChassisSpeeds,
      BooleanSupplier mirrorTrajectory,
      Subsystem... requirements) {
    var timer = new Timer();
   ...
   */
  public static AftershockChoreoControlFunction choreoSwerveController(
   ...
   */
  public static AftershockChoreoControlFunction choreoSwerveController(
      PID xController, PID yController, PID rotationController) {
    // rotationController.enableContinuousInput(-Math.PI, Math.PI);
    return (pose, referenceState) -> {
      double xFF = referenceState.velocityX;
      double yFF = referenceState.velocityY;
      ...
      double xFeedback = xController.update(pose.getX(), referenceState.x);
      double yFeedback = yController.update(pose.getY(), referenceState.y);
      double rotationFeedback =
          rotationController.updateRotation(pose.getRotation().getRadians(), referenceState.heading);
